#About the school management application
...
To run this application, we need maven a powerful project management tool
that is based on POM (project object model).

To setup and run maven in our system do the following steps:
1. Install maven 
2. Setup the classpath in environmental variable
3. In the command prompt, type a) mvn --version (to check the version of the installed maven)
                               b) mvn clean (to build our project)
                               c) mvn exec:java -Dexec.mainClass="MainController" (to run our project) 
